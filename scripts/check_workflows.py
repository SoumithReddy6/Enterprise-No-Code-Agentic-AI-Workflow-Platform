...
public HTTP GET. Nothing touches the signed-in workspace. This is a diagnostic report, not a
pass/fail gate: known gaps are printed as such (see docs/production-gap-analysis.md).
...
async def main():
    ...
    with TemporaryDirectory(prefix='relay-workflows-') as d:
        ...
        if shutil.which('docker'):
            await execute('4 agent_python_tool',wf('Calc',[CHAT,{**agent('agent',system='You have a python calculator tool that evaluates a Python arithmetic expression given as input and prints the result.'),'inputs':{'input':'input.message'}},{'id':'calc','type':'tool_python','config':{'code':"print(eval(input_text,{'__builtins__':{}},{}))",'timeout_seconds':10}},out()],flow(('input','agent'),('agent','out'))+[tool_edge('calc','agent')]),'Compute 15% of 240, then add 17. Use the calculator tool and report the final number.',lambda r:'53' in r['output'],'KNOWN GAP: tools carry no argument schema and a tool error aborts the run')
        else:print('\n- 4 agent_python_tool skipped: Docker not available')
        await execute('5 agent_retrieve_tool',wf('AgentSearch',[CHAT,{**agent('agent',system='You can search the employee handbook with the attached retrieve tool.'),'inputs':{'input':'input.message'}},retrieve('search'),out()],flow(('input','agent'),('agent','out'))+[tool_edge('search','agent')]),'Search the handbook: how many paid vacation days do full-time employees accrue per year?',lambda r:'22' in r['output'],'KNOWN GAP: tool results are raw dicts; no citation contract on the tool path')
